...
def main():
    ...
    if not records:
        run_maintain = input(
            "Would you like to run 'maintain.py' to setup projects? (yes/no): "
        )
    ...
    while True:
        ...
        if choice == 1:
            ...
            if customer is not None:
                ...
                if project is not None:
                    if "start_record" in locals():
                        stop_time_tracking(
                            datetime.datetime.now().strftime("%Y_%m_%d.csv"),
                            start_record,
                        )
                    start_record = start_time_tracking(
                        datetime.datetime.now().strftime("%Y_%m_%d.csv"),
                        customer,
                        project,
                    )
            ...
            if "start_record" in locals():
                stop_time_tracking(
                    datetime.datetime.now().strftime("%Y_%m_%d.csv"), start_record
                )
                del start_record
            else:
