...
def write_report_to_file(filename: str, records: List[Dict[str, str]]) -> None:
    ...
    report.append("-----------------")
    for customer, time in sorted(
        customer_time.items(), key=lambda item: item[0].lower()
    ):
...
def print_report(records: List[Dict[str, str]]) -> None:
    ...
    print("-----------------")
    for customer, time in sorted(
        customer_time.items(), key=lambda item: item[1], reverse=True
    ):
    ...
    for project, time in sorted(
        project_time.items(), key=lambda item: item[1], reverse=True
    ):
