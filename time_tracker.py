...
def start_time_tracking(filename: str, customer: str, project: str) -> Dict[str, str]:
    ...
    print(
        f"Started time tracking for {customer} / {project} at {start_time.strftime('%H:%M:%S')}."
    )
...
def stop_time_tracking(filename: str, start_record: Dict[str, str]) -> None:
    ...
    duration = (
        stop_time
        - datetime.datetime.strptime(start_record["Starttime"], "%Y-%m-%d %H:%M:%S")
    ).seconds // 60
    time_record = {
    ...
    print(
        f"Stopped time tracking for {time_record['Kunde']} / {time_record['Projekt']} at {stop_time.strftime('%H:%M:%S')}. Duration: {duration} minutes."
    )
